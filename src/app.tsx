...
export const LanguageShadow = memo(function LanguageShadow() {
  ...
  useEffect(() => {
    ...
    nlpEngine.init().then(() => {
      ...
      axios.get(`http://localhost:${port}/gpu`).then(response => {
  ...
  return <Window title={title} size={{
    width: 300,
    height: 360
  }} onInit={onInit} onClose={onClose}>
			<Grid style={{
      layout: containerLayout
    }}>
				<Grid style={{
        area: containerLayout.areas.control,
        layout: controlLayout
      }}>
					{isReady && !isError ? <>
							<Grid style={{
            area: controlLayout.areas.measure
          }}>
								<Button text="选择识别区" iconInfo={{
              name: "measure",
              size: 16
            }} onClick={onMeasure}></Button>
							</Grid>
							<Grid style={{
            area: controlLayout.areas.reset
          }}>
								<Button text="重置识别区" onClick={onReset}></Button>
							</Grid>
							<Grid style={{
            area: controlLayout.areas.topmost
          }}>
								<CheckBox text="字幕置顶" value={CheckValue.Checked} onCheck={onSetTopMost}></CheckBox>
							</Grid>
							<Grid style={{
            area: controlLayout.areas.fontSizeLabel
          }}>
								<Label text="字体大小"></Label>
							</Grid>
							<Grid style={{
            area: controlLayout.areas.fontSize,
            margin: "10dpx 0 10dpx 0"
          }}>
								<ScrollBar min={10} max={50} value={14} /** default value */ onScrolling={onSetFontSize}></ScrollBar>
							</Grid>
							<Grid style={{
            area: controlLayout.areas.fontSizeValue
          }}>
								<Label text={`${fontSize}`}></Label>
							</Grid>
							<Grid style={{
            area: controlLayout.areas.snow
          }}>
